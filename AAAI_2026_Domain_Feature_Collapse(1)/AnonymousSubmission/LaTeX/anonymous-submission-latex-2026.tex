...
Surprisingly, there is a fatal flaw in OOD detection that only occurs in single domain settings. This paper introduces the concept of and provides the first formal proof of \textbf{domain feature collapse}. Our main theoretical contribution establishes that domain feature loss is not an empirical observation but a mathematically inevitable consequence of information bottleneck optimization under single-domain supervised learning. Through rigorous information-theoretic analysis, we prove that any model minimizing a standard supervised loss will necessarily learn representations with $I(\rvx_\rvd; \rvz) = 0$, where $\rvx_\rvd$ represents domain features and $\rvz$ the learned representation. This result is counterintuitive: better class-specific optimization leads to worse domain robustness. The theorem explains why state-of-the-art methods struggle with out-of-domain detection and provides predictive power to identify when and how severely domain feature collapse will occur. This leads to a situation where OOD detection relies solely on class-specific features, while ignoring domain-specific features (e.g., knowing that an image is an X-ray does not help in detecting the disease depicted by the X-ray, but would help in OOD detection). Unfortunately, this failure results in higher OOD detection error rates when the ID set is single domain versus multi-domain. Since this mode of OOD detection failure rarely occurs for multi-domain ID sets, it is difficult to identify in the commonly used OOD benchmarks.
...
\begin{itemize}
\item \textbf{Domain Feature Collapse (Theoretical)}: We provide the first formal proof that single-domain supervised learning inevitably leads to domain feature loss under information bottleneck optimization. This is not common knowledge—the proof requires decomposing mutual information under independence constraints, analyzing the optimization landscape, and establishing that all alternative solutions either violate sufficiency or have strictly higher loss. The theorem reveals a fundamental trade-off: optimal class prediction necessitates domain information loss, a counterintuitive result with significant implications for OOD detection. We extend this result using Fano's inequality to quantify the severity of partial domain feature collapse in practical scenarios.

\item \textbf{Domain Filtering (Architectural)}: We introduce a theoretically-motivated solution that addresses the root cause identified by our theorem: the $I(\rvx_\rvd; \rvz) = 0$ problem. Our key insight is architectural, not algorithmic—we demonstrate that the choice of representation space (pretrained vs. supervised) fundamentally determines OOD detection capability. Domain filtering is a method-agnostic framework that works with diverse base detectors (MSP, Energy, Mahalanobis, KNN, ReAct, Scale, NCI), representing a paradigm shift from "better algorithms" to "better representation spaces" for OOD detection.
...
We use the following $11$ single-domain datasets as the in-distribution training data to evaluate the impact of domain feature collapse. Each dataset represents a narrow, homogeneous domain where all training samples share common domain features $\rvx_\rvd$ (e.g., all images are satellite imagery, or all images are medical pathology slides, or all images are playing cards). This single-domain characteristic is critical: it ensures that domain features $\rvx_\rvd$ are independent of class features $\rvx_\rvy$ and thus subject to compression during supervised learning, as predicted by Theorem \ref{mainbodygenloss}. These datasets often contain class features that are quite subtle and difficult for pretrained models to pick up. Additional descriptions and sample images are provided in Appendix \ref{dataset}.
